...
spinlock_t scull_w_lock = SPIN_LOCK_UNLOCKDE;
uid_t	   scull_w_owner;
...
int scull_open (struct inode *inode, struct file *filp)
{
	...
	if (scull_w_count == 0)
		scull_w_owner = current->uid;	/* 获得所有者 */
	scull_w_count++;
	spin_unlock(&scull_w_lock);
...
static int __init scull_init(void)
{
	...
	if (scull_major) {
		...
		result = register_chrdev_region(dev, scull_nr_devs, "scullwuid");
	} else {
